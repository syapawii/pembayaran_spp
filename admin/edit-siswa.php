...
<form method="post" action="?url=proses-edit-siswa&nisn=<?= $nisn; ?>">
    ...
    <div class="form-group mb-2">
        ...
        <input value="<?= $data['no_telpon'] ?>" type="number" name="no_telpon" class="form-control" required>
    </div>
    <div class="form-group mb-2">
        <label>SPP</label>
        <select name="id_spp" class="form-control" required>
            <option value="<?= $data['id_spp'] ?>"> <?= $data['tahun']; ?> |
                <?= number_format($data['nominal'],0,',','.'); ?> </option>
            <?php
            include'../koneksi.php';
            ...
            <option value="<?= $data_spp['id_spp'] ?>"> <?= $data_spp['tahun']; ?> |
                <?= number_format($data_spp['nominal'],3,',','.'); ?> </option>
            <?php } ?>
